...
変更線を3回試した。目標値と制約値は変更していない。Step 20でレビュー側の追加探索を反映した。
...
中央線（2本目）の終点のみを`[59, 24]`（Q16.16: `[3866624, 1572864]`）へ変更した解を、`docs/reports/data/step-18-solution.json`として追加した。`stateHash`は`9e564fe2`である。

| 速さ | 量 | 集中（距離0の密度比） | 手数 |
|---:|---:|---:|---:|
| 266 step | 2,147,895 | 41.8% | 3本 / 14点 |

3回の試行はいずれも中央線をupperまたはlowerへ直接つなぐ変更であり、中間帯を探索していなかった。終点y=24はupper 64.26% / 場外0% / 到達率58%でクリアする。
...
- 目標帯は達成可能である。Step 20で終点y=24のクリア解を追加し、4軸を確認した。

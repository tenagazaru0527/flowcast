...
const OUTPUT = "docs/reports/data/step-06-sweep.csv";
const INPUT_NAMES = ["straight", "distributed", "detour"];
const SAMPLE_X = [4, 8, 12, 16, 20, 24, 28, 32, 36, 40, 44, 48, 52, 56, 60];
const POINTS = [1, 3, 9].flatMap((sourceWidth) => (
  [1, 3, 5, 9, 17].flatMap((sinkWidth) => [
    { sourceWidth, sinkWidth, ratio: "4:1", advectionWeight: Q, diffusionWeight: Q >> 2 },
    { sourceWidth, sinkWidth, ratio: "16:1", advectionWeight: Q, diffusionWeight: Q >> 4 },
  ])
));
...
function simulate(lines, point) {
  return runSimulation({
    lines,
    source: createWideSource(point.sourceWidth),
...
function serialize(results) {
  ...
  const headers = [
    "sourceWidth", "sinkWidth", "ratio", "advectionWeight", "diffusionWeight", "inputName", "stateHash",
    "w0", "coherenceLength", "coherenceLengthSigma", ...sampledHeaders,
  ...
  for (let resultIndex = 0; resultIndex < results.length; resultIndex += 1) {
    ...
    for (let inputIndex = 0; inputIndex < INPUT_NAMES.length; inputIndex += 1) {
      ...
      rows.push([
        result.point.sourceWidth, result.point.sinkWidth, result.point.ratio,
        result.point.advectionWeight, result.point.diffusionWeight, INPUT_NAMES[inputIndex], simulation.stateHash,
        values.w0, values.coherenceLength, values.coherenceLengthSigma, ...sampled,
